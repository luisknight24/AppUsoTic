...
class creditoMostrarHome {
  ...
  final String baseUrl11 = "https://apicredito2-8.onrender.com";
  ...
  Future<void> connectSignalR() async {
    ...
    _connection.on('CreditoActualizado', (args) {
      ...
      try {
        ...
        debugPrint(
          "✅ Evento procesado | cache size: ${_cacheCreditos!.length}",
        );
      } catch (e, s) {
  ...
  Future<void> getCreditos({bool forceRefresh = false}) async {
    ...
    try {
      ...
      cargandoNotifier.value = false;
      debugPrint(
        "🟢 [getCreditos] créditos cargados: ${creditosNotifier.value?.length}",
      );
    }
  }
  ...
  void _actualizarCreditoDesdeEvento(CreditoMostrarDTO nuevo) {
    ...
    creditosNotifier.value = List.from(_cacheCreditos!);
    debugPrint(
      "✅ Crédito actualizado | id: ${nuevo.id} | montoPendiente: ${nuevo.montoPendiente} | proximaCuotaStr: ${_cacheCreditos![index].proximaCuotaStr}",
    );
  }
